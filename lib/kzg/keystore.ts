...
export class KZGKeyStoreFactory implements KeyStoreFactory {
    ...
    async deploy(pubKeys: PubKey[], runner: ContractRunner): Promise<KeyStore> {
        const ksFactory = new KZGKeyStore__factory();
        ...
        const ks = new KZGKeyStore(ksAddr, this.parameters, pubKeys, runner);
        return ks;
    }
